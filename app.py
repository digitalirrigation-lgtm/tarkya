...
# ---------- TABS ----------
tab1, tab2, tab3, tab4 = st.tabs(["📚 Manage Data", "🎯 Apply", "📄 Export", "📝 Templates"])
        ...
        for display_name, filename in template_files.items():
            ...
            if os.path.exists(file_path):
                ...
                with st.expander(f"📄 {display_name}", expanded=False):
                    st.text_area(f"{display_name}", content, height=200, key=f"template_{filename}")
                    st.download_button(
                ...
                st.warning(f"File {filename} not found in the templates folder.")
    else:
        st.error("❌ Templates folder not found. Please ensure the 'templates' folder is in the app directory.")
        st.info("""
        **To fix this:**
